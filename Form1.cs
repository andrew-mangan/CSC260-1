...
namespace Hangman
{
    ...
    public partial class Form1 : Form
    {
        ...
        public void DisplayTruth(char[] letters, string word)
            {
        ...
        {
            if (tbxWord.Text.Length > 8) { MessageBox.Show("That is too big for this game.  Try again."); }  //test to see if it is too big.
            else if (tbxWord.Text != null)
            {
                var Testword = new TakeWord("UNKNOWN", tbxWord.Text, tbxWord.Text.Length);  // checks to see if the text box is empty
                GLetters = Testword.Letters;
                DisplayWord(Testword.Letters, Testword.CodeWord);
                Radios(Testword.Letters.Length);
            }
        ...
        public void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            switch (failstep)
            {
                ...
                case 11:
                    ptb11.Visible = true;
                    DisplayTruth(GLetters, GLetters.ToString());
                    MessageBox.Show("You have failed this City Mr. White!");
